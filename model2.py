...
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = h5_task0, labels = labels, name = "cross_entropy_mean")) + lamda*l2_t0 
global_step = tf.Variable(0)

train_op = tf.train.AdamOptimizer(1e-4).minimize(loss,global_step = global_step)
correct = tf.equal(tf.argmax(h5_task0,1), tf.argmax(labels,1), name = 'Correct_prediction')
